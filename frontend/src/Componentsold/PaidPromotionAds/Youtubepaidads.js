...
import { fetchpaidyoutubepage, base_api } from "../Axios/Axios";


function Youtubepaidads() {
  const [paidyoutubepage, setPaidyoutubepage] = useState([]);
  const [isBackendError, setIsBackendError] = useState(false);

  useEffect(() => {
    const fetchData = async () => {
      try {
        const paidyoutubepageData = await fetchpaidyoutubepage();
        setPaidyoutubepage(paidyoutubepageData);
      } catch (error) {
        console.error("Error fetching youtube  paid  data:", error);
  ...
  if (isBackendError || !paidyoutubepage || !paidyoutubepage.length) {
    return (
      <div>
        <div className="container-paid">
          <div className="pagein-content">
            ...
            <div className="row row-cols-1 row-cols-md-3 g-4 second-main2">
                ...
                <div className="col-lg-4 col-md-6 col-sm-12">
                  <div className="card">
                    <img src="/Facebook_images/fbsaaral.jpg" className="card-img-top" height="300" width="200" alt="..." />
                    <div className="card-body">
